...
class AdvancedTestSuite(unittest.TestCase):
  ...
  def test_write_read_same_key(self):
    ...
    wkey128 = kskit.crypto.generate_qr_key(f128, 128)
    wkey256 = kskit.crypto.generate_qr_key(f256, 256)
    wkey512 = kskit.crypto.generate_qr_key(f512, 512)
    rkey128 = kskit.crypto.get_qr_key(f128)
    rkey256 = kskit.crypto.get_qr_key(f256)
    rkey512 = kskit.crypto.get_qr_key(f512)
    with self.subTest():
  ...
  def test_keys_right_length(self):
    ...
    f256 = os.path.join(self.test_dir, f"{name}256.png")
    f512 = os.path.join(self.test_dir, f"{name}512.png")
    wkey32 = base64.decodebytes(kskit.crypto.generate_qr_key(f32, 32).encode("ASCII"))
    wkey128 = base64.decodebytes(kskit.crypto.generate_qr_key(f128, 128).encode("ASCII"))
    wkey256 = base64.decodebytes(kskit.crypto.generate_qr_key(f256, 256).encode("ASCII"))
    wkey512 = base64.decodebytes(kskit.crypto.generate_qr_key(f512, 512).encode("ASCII"))
    with self.subTest():
  ...
  def test_crypt_decrypt_same_file(self):
    ...
    key = kskit.crypto.generate_qr_key(keyfile, 32)
    ...
    #encrypt and decrypt
    kskit.crypto.encrypt(plain, crypted, key)
    kskit.crypto.decrypt(crypted, replain, key)
    #obtaining same file
